module concatenation_operator();

  reg[3:0] r_VAL_1 = 4'b0111;
  reg[3:0] r_VAL_2 = 4'b1100;
  wire[7:0] w_CAT;

  reg[3:0] r_UNSIGNED = 4'b0010;
  reg signed [3:0] r_SIGNED = 4'b1110;
  wire[7:0] w_CAT_2;
  wire[15:0] w_CAT_3;

  reg r_CLOCK = 1'b0;
  reg [7:0] r_SHIFT_REG = 8'h01;

  // demonstrates a common concatenation
  assign w_CAT = {r_VAL_1, r_VAL_2};

  assign w_CAT_2 = {r_SIGNED, r_UNSIGNED};
  assign w_CAT_3 = {w_CAT, w_CAT_2};

  initial
    begin
      $dumpfile("concatenation.vcd");
      $dumpvars(0, concatenation_operator);
      #5;
      $display("Concatenation of %h and %h is %h", r_VAL_1, r_VAL_2, w_CAT);
      #20;
      $display("Concatenation of %h and %h is %h", r_SIGNED, r_UNSIGNED, w_CAT_2);
      #20;
      $display ("Concatenation of two 4-bit regs into a 16-bit reg: %h", w_CAT_3);
      #20;
      $display("Complete");
      $finish;   
    end

  // generate a clock
  always #1 r_CLOCK = ~r_CLOCK;

  // demonstrate shifting of a 1 through an 8 bit register
  always @ (posedge r_CLOCK)
  begin
    r_SHIFT_REG <= {r_SHIFT_REG[6:0], r_SHIFT_REG[7]};
  end

endmodule